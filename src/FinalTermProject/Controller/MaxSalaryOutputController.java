...
public class MaxSalaryOutputController {
    private Main main;
    ...
    private void initializeColumns(){
        ...
        maxSalaryOutputTable.getColumns().addAll(nameColumn, countryColumn, ageColumn, heightColumn, positionColumn, numberColumn, salaryColumn);
    }

    public ObservableList<Player> getPlayers(){
        ObservableList<Player> players = FXCollections.observableArrayList();

        Collections.sort(ClubPlayerList, Player.PlayerSalary);

        Player plr = new Player();
        plr = ClubPlayerList.get(0);

        maximum_Salary = plr.getWeeklySalary();

        for(Player p: ClubPlayerList){
            if(p.getWeeklySalary() == maximum_Salary){
                players.add(p);
            }
        }
        return players;
    ...
    public void load(){
        ...
        maxSalaryOutputTable.setEditable(true);
        maxSalaryOutputTable.setItems(getPlayers());
    }
    ...
    @FXML
    private TableView<Player> maxSalaryOutputTable;
