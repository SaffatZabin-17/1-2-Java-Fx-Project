...
public class MaxAgeOutputController {
    private Main main;
    ...
    private void initializeColumns(){
        ...
        maxAgeOutputTable.getColumns().addAll(nameColumn, countryColumn, ageColumn, heightColumn, positionColumn, numberColumn, salaryColumn);
    }

    public ObservableList<Player> getPlayers(){
        ObservableList<Player> players = FXCollections.observableArrayList();

        Collections.sort(ClubPlayerList, Player.PlayerAge);

        Player plr = new Player();
        plr = ClubPlayerList.get(0);

        maximum_age = plr.getAge();

        for(Player p: ClubPlayerList){
            if(p.getAge() == maximum_age){
                players.add(p);
            }
        }
        return players;
    ...
    public void load(){
        ...
        maxAgeOutputTable.setEditable(true);
        maxAgeOutputTable.setItems(getPlayers());
    }
    ...
    @FXML
    private TableView<Player> maxAgeOutputTable;
